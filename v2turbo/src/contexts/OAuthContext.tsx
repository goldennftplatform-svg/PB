...
export type OAuthProvider = 'twitter' | 'google' | 'discord' | 'github' | 'farcaster';
...
export function OAuthProvider({ children }: { children: ReactNode }) {
  ...
  const refreshLinks = useCallback(async () => {
    ...
    try {
      ...
      const providers: OAuthProvider[] = ['twitter', 'google', 'discord', 'github', 'farcaster'];
      const linkPromises = providers.map(async (provider) => {
        try {
          const response = await fetch(getPartyServerHttpUrl(`/api/social-links/${provider}`), {
  ...
  const connect = useCallback(
    async (provider: OAuthProvider) => {
      ...
      const callbackUrl = getPartyServerHttpUrl('/api/oauth/callback');
